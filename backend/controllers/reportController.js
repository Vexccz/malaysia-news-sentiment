...
function drawIntroduction(doc, stats) {
  ...
  bodyText(doc, 'The primary objectives of this analysis are threefold: (1) to quantify the overall sentiment distribution across the collected articles and identify dominant narrative patterns, (2) to identify temporal and geographic patterns in sentiment expression that may correlate with real-world events, and (3) to assess the reliability of automated sentiment classification through confidence score analysis and discuss the implications for data-driven decision-making.');
  bodyText(doc, 'The remainder of this report is structured as follows. Section 2 presents the executive summary and key findings. Section 3 describes the methodology employed for data collection and classification. Section 4 presents the detailed results and analysis. Sections 5 through 9 cover key findings, recommendations, article listings, technical appendices, and concluding remarks respectively.');
}
...
function drawSourceAnalysis(doc, stats) {
  ...
  bodyText(doc, 'The dataset encompasses ' + stats.sources.length + ' distinct news sources. Table 2 lists the top contributors by article volume. ' + (topSource ? topSource.name + ' leads with ' + topSource.n + ' articles (' + pct(topSource.n, stats.total) + '% of total)' + (stats.sources.length > 1 ? ', followed by ' + stats.sources[1].name + ' (' + stats.sources[1].n + ' articles)' : '') + '. ' : '') + 'Source diversity is a critical factor in ensuring representativeness, as concentration from a single outlet may introduce editorial bias that skews the overall sentiment profile.');
  bodyText(doc, (topSource ? topSource.name + ' sentiment profile: ' + topSource.p + ' positive, ' + topSource.ng + ' negative, ' + topSource.nt + ' neutral. ' + (topSource.ng > topSource.p ? 'The negative skew in this dominant source warrants consideration when interpreting overall corpus sentiment.' : 'The balanced or positive profile provides confidence in representativeness.') : 'Varied editorial stances across the Malaysian media landscape.'));
...
function drawConclusion(doc, stats) {
  ...
  bodyText(doc, 'This sentiment analysis of ' + stats.total + ' Malaysian news articles' + (topic ? ' related to “' + topic + '”' : '') + ' reveals ' + (dominantSentiment === 'positive' ? 'a predominantly positive media narrative' : dominantSentiment === 'negative' ? 'a predominantly negative media narrative' : 'a balanced media narrative') + ' during the observed period of ' + stats.dateRange + '. The analysis encompassed ' + stats.sources.length + ' distinct news sources spanning ' + stats.states.length + ' geographic regions, providing a comprehensive cross-section of the Malaysian media landscape.');
  bodyText(doc, 'The sentiment distribution of ' + pct(stats.posN, stats.total) + '% positive, ' + pct(stats.negN, stats.total) + '% negative, and ' + pct(stats.neuN, stats.total) + '% neutral classifications ' + (dominantSentiment === 'negative' ? 'suggests a media environment reflecting either genuine adverse conditions or a tendency towards critical reporting.' : dominantSentiment === 'positive' ? 'indicates either genuinely favourable developments or potential source selection bias.' : 'demonstrates a balanced media ecosystem indicative of healthy media plurality.'));

  subsectionTitle(doc, '10.2', 'Implications');
  bodyText(doc, (topSource ? 'The concentration of coverage from ' + topSource.name + ' (' + pct(topSource.n, stats.total) + '%) highlights individual outlet influence. ' : '') + 'Geographic distribution reveals ' + (stats.states.length > 5 ? 'broad but uneven national coverage.' : 'limited diversity requiring supplementation.') + ' Classification confidence of ' + (stats.avgConf * 100).toFixed(1) + '% ' + (stats.avgConf >= 0.7 ? 'validates the dual-model approach for Malaysian multilingual analysis.' : 'suggests manual review for high-stakes decisions.'));

  subsectionTitle(doc, '10.3', 'Limitations');
  bodyText(doc, 'Limitations include: (1) RSS-based corpus may underrepresent non-digital outlets, (2) automated classification cannot capture sarcasm or irony, (3) geographic NER may miss implicit location references, and (4) the analysis period may not reflect longer-term cycles or seasonal patterns.');
