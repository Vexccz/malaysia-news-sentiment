...
import { Globe, Lock, Newspaper, Network, Radio, TrendingUp, FileText, Bell, ShieldCheck, BarChart3, FileDown, Clock, Share2, Settings, Shield, ChevronLeft, Search, Copy, Check, Play, ExternalLink, AlertTriangle } from 'lucide-react';
import api from '../services/api';

const API_BASE = import.meta.env.VITE_API_BASE || 'http://localhost:5001/api/v1';
const ICON_MAP = { Globe, Lock, Newspaper, Network, Radio, TrendingUp, FileText, Bell, ShieldCheck, BarChart3, FileDown, Clock, Share2, Settings, Shield, AlertTriangle, Search };
const CatIcon = ({ name, className = 'w-5 h-5' }) => { const C = ICON_MAP[name]; return C ? <C className={className} strokeWidth={1.5} /> : null; };
...
const CodeBlock = ({ code, language = 'json' }) => {
  ...
  return (
    <div className="relative group">
      <pre className="bg-[#1A1A1A] dark:bg-black/50 text-gray-100 rounded-sm p-4 text-xs overflow-x-auto font-mono leading-relaxed border border-gray-800 dark:border-gray-700">
        <code>{text}</code>
      </pre>
      <button
        onClick={handleCopy}
        className="absolute top-2 right-2 px-3 py-1.5 bg-white/10 text-white/70 text-xs opacity-0 group-hover:opacity-100 transition-all hover:bg-white/20 hover:text-white font-medium"
      >
        {copied ? 'Copied' : 'Copy'}
      </button>
    </div>
  ...
  return (
    <div className="space-y-2">
      <button
        onClick={tryIt}
        disabled={loading}
        className="px-4 py-2 bg-accent text-white text-xs font-medium hover:bg-accent/90 disabled:opacity-50 transition-all flex items-center gap-2"
      >
        {loading ? (
...
const SECTION_ROUTES = [
  { path: '', id: 'overview', label: 'Overview', icon: 'Search' },
  { path: 'authentication', id: 'authentication', label: 'Authentication', icon: 'Lock' },
  { path: 'errors', id: 'errors', label: 'Error Codes', icon: 'AlertTriangle' },
  { path: 'realtime', id: 'realtime', label: 'Real-time / SSE', icon: 'Radio' },
...
const Sidebar = ({ activeSection, mobileMenuOpen = false, setMobileMenuOpen = () => {}, navigate }) => {
  ...
  return (
    <nav className={`w-60 shrink-0 sticky top-[4.5rem] self-start max-h-[calc(100vh-5rem)] overflow-y-auto pr-2 transition-transform duration-300 ${mobileMenuOpen ? 'md:translate-x-0 fixed left-0 top-0 bottom-0 z-50 translate-x-0 bg-[#fafaf9] dark:bg-[#0f0f0f] pt-6 px-4 border-r border-[#eee] dark:border-[#2a2a2a]' : 'md:block hidden'}`}>
      <div className="bg-[#FAF8F3] dark:bg-[#1A1917] border border-[#1A1A1A]/10 dark:border-[#FAF8F3]/10 rounded-sm p-4 space-y-1">
        {/* Logo/Title */}
        <div className="px-3 pb-3 mb-2 border-b border-[#eee] dark:border-[#2a2a2a]">
          <p className="text-xs font-bold text-gray-900 dark:text-white font-display">API Reference</p>
        ...
        <div className="space-y-0.5">
          {generalItems.map(item => (
            <a
              ...
              className={`w-full text-left px-3 py-2 text-xs font-medium transition-all duration-200 flex items-center gap-2.5 ${
            ...
            >
              <span className="text-sm w-5 text-center"><CatIcon name={item.icon} className="w-4 h-4" /></span>
        ...
        <div className="pt-3 pb-2 px-3">
          <span className="text-[10px] uppercase tracking-[0.2em] font-bold text-gray-400 dark:text-gray-500">Endpoints</span>
        </div>
          ...
          {categoryItems.map(item => {
            ...
            return (
              ...
              >
                ...
                {cat && <span className="text-[10px] px-1.5 py-0.5 bg-gray-100 dark:bg-white/5 text-gray-400">{cat.endpoints.length}</span>}
              </a>
            );
          })}
        </div>
            ...
            <div className="p-4 space-y-4">
              ...
              {endpoint.params && endpoint.params.length > 0 && (
                <div>
                  ...
                  <div className="border border-[#eee] dark:border-[#2a2a2a] overflow-hidden">
    ...
    <div className="space-y-8">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
        <div className="flex items-center gap-3 flex-wrap">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white flex items-center gap-3 font-['Playfair_Display']">
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="m18 16 4-4-4-4"/><path d="m6 8-4 4 4 4"/><path d="m14.5 4-5 16"/>
            </svg>
            API Documentation
          </h1>
          <span className="px-2.5 py-1 text-[10px] font-bold bg-accent/10 text-accent border border-accent/20">v1.0</span>
          <span className="px-2.5 py-1 text-[10px] font-bold bg-green-50 dark:bg-green-500/10 text-green-600 dark:text-green-400 border border-green-200 dark:border-green-500/20 flex items-center gap-1">
        ...
        {/* Getting Started */}
        <div className="mt-6 border-l-[3px] border-l-red-600 bg-[#FAF8F3] dark:bg-[#1A1917] border border-[#1A1A1A]/10 dark:border-[#FAF8F3]/10 rounded-sm p-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <motion.div
            ...
            className="bg-white dark:bg-[#1a1a1a] border border-[#1A1A1A]/10 dark:border-[#FAF8F3]/10 rounded-sm p-4"
          >
      ...
      {/* Rate Limiting */}
      <div className="border-l-[3px] border-l-amber-500 bg-[#FAF8F3] dark:bg-[#1A1917] border border-[#1A1A1A]/10 rounded-sm p-5">
        <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-2">Rate Limiting</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {apiCategories.map(cat => (
            <Link
              key={cat.id}
              to={`/api-docs/${cat.id}`}
              className="bg-white dark:bg-[#1a1a1a] border border-[#1A1A1A]/10 dark:border-[#FAF8F3]/10 rounded-sm p-4 hover:border-l-[3px] hover:border-l-red-600 transition-all group"
            >
              <div className="flex items-center gap-3">
                <span className="text-xl"><CatIcon name={cat.icon} className="w-5 h-5" /></span>
                ...
                <span className="text-xs px-2 py-0.5 bg-gray-100 dark:bg-white/5 text-gray-400">{cat.endpoints.length}</span>
              </div>
            </Link>
          ))}
        </div>
    ...
    <div className="space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center gap-3 mb-2">
          <span className="text-2xl"><CatIcon name={category.icon} className="w-5 h-5" /></span>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{category.title}</h1>
          <span className="text-xs text-gray-400 bg-gray-100 dark:bg-white/5 px-2 py-0.5">
...
const ApiDocs = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F3] dark:bg-[#12110F] transition-colors">
      {/* Standalone Navbar */}
      <nav className="sticky top-0 z-50 bg-[#FAF8F3] dark:bg-[#12110F] border-b-2 border-[#1A1A1A] dark:border-[#FAF8F3]">
        <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2 text-sm font-bold text-gray-900 dark:text-white hover:text-accent transition-colors">
            ...
            MY News <span className="italic text-red-600">Sentiment</span>
          </Link>
          <div className="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
            <Link to="/" className="hover:text-accent transition-colors uppercase tracking-wider text-[10px]">Home</Link>
            <Link to="/login" className="hover:text-accent transition-colors uppercase tracking-wider text-[10px]">Login</Link>
            <Link to="/register" className="px-3 py-1.5 bg-[#1A1A1A] dark:bg-[#FAF8F3] text-[#FAF8F3] dark:text-[#1A1A1A] font-medium hover:opacity-90 transition-colors uppercase tracking-wider text-[10px]">Get Started</Link>
          </div>
        </div>
      </nav>
      ...
      <button
        ...
        className="md:hidden fixed bottom-6 right-6 z-50 w-14 h-14 bg-accent text-white border-2 border-accent flex items-center justify-center hover:bg-accent/90 transition-all active:scale-95"
      >
