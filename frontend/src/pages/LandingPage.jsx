...
import { SpringCounter, MagneticButton, CursorTrail, RevealOnScroll } from '../components/kinetics';
...
const sectionHead = (eyebrow, title, copy) => (
  <div className="mb-10 md:mb-14">
    ...
    {copy && <p className="mt-4 max-w-2xl text-sm sm:text-base leading-7 text-ink-muted dark:text-ink-faint dark:text-[#B8B3A7]">{copy}</p>}
  </div>
);
...
const MalaysiaSignalMap = () => {
  ...
  return (
    <div className="relative mt-5 border-t border-ink/10 dark:border-paper/10 pt-4">
      <div className="flex justify-between items-center mb-1 text-[8px] uppercase tracking-[.18em] text-ink-faint dark:text-[#B8B3A7]">
    ...
    <nav className="fixed inset-x-0 top-0 z-50 border-b border-ink/15 dark:border-paper/15 bg-paper/90 dark:bg-paper-dark/90 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto h-16 px-5 sm:px-6 flex items-center justify-between">
        <Link to="/" className="flex items-baseline gap-2">
          ...
          <span className="hidden lg:inline text-[9px] tracking-[.2em] text-ink-faint dark:text-[#B8B3A7]">EST. 2026</span>
        </Link>
        <div className="hidden md:flex items-center gap-7">
          {links.map(([label, href]) => <a key={href} href={href} className="text-[11px] tracking-[.16em] uppercase text-ink-muted dark:text-ink-faint dark:text-[#B8B3A7] hover:text-accent">{label}</a>)}
          <Link to="/about" className="text-[11px] tracking-[.16em] uppercase text-ink-muted dark:text-ink-faint dark:text-[#B8B3A7] hover:text-accent">About</Link>
        </div>
  ...
  return (
    <Motion.div id="pulse" variants={fadeUp} whileHover={{ y: -4 }} transition={{ duration: .35, ease: EASE }} className="relative text-left border border-ink/20 dark:border-paper/20 bg-paper-card dark:bg-paper-dark-card shadow-[10px_10px_0_rgba(199,47,53,.12)] overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-ink/15 dark:border-paper/15 bg-ink/[.025] dark:bg-paper/[.025]">
        ...
        <span className="text-[9px] text-ink-faint dark:text-[#B8B3A7] uppercase tracking-widest">KUL · MYT</span>
      </div>
      <div className="grid md:grid-cols-[.9fr_1.1fr]">
        <div className="p-6 sm:p-8 border-b md:border-b-0 md:border-r border-ink/15 dark:border-paper/15">
          <p className="text-[10px] tracking-[.2em] text-ink-muted dark:text-ink-faint dark:text-[#B8B3A7] uppercase">Overall media signal</p>
          ...
          <div className="flex justify-between mt-2 text-[9px] tracking-wider text-ink-faint dark:text-[#B8B3A7]"><span>46% POS</span><span>34% NEU</span><span>20% NEG</span></div>
          <MalaysiaSignalMap />
        </div>
        <div>
          <div className="px-5 py-3 border-b border-ink/10 dark:border-paper/10 flex justify-between text-[9px] uppercase tracking-[.18em] text-ink-faint dark:text-[#B8B3A7]"><span>Trending now</span><span>Signal</span></div>
          {pulseTopics.map((item, i) => (
            <Motion.button layout key={item.topic} onClick={() => setActive(i)} animate={{ x: active === i ? 4 : 0 }} className={`relative w-full grid grid-cols-[30px_1fr_70px_50px] items-center gap-2 px-5 py-3.5 text-left border-b border-ink/[.07] dark:border-paper/[.07] transition-colors ${active === i ? 'bg-accent/[.06]' : 'hover:bg-ink/[.025] dark:hover:bg-paper/[.025]'}`}>
              ...
              <span className="font-display text-xs text-ink-faint dark:text-[#B8B3A7]">{String(i+1).padStart(2,'0')}</span><span className="text-sm font-semibold">{item.topic}</span><Sparkline values={item.spark} positive={item.score > 0} className="w-full h-7"/><span className={`text-right text-xs font-bold ${item.score > 0 ? 'text-[#16855b]' : 'text-accent'}`}>{item.score > 0 ? '+' : ''}{item.score}</span>
            </Motion.button>
          ))}
          <div className="px-5 py-3 text-[9px] uppercase tracking-widest text-ink-faint dark:text-[#B8B3A7] flex justify-between"><span>{articleCount.toLocaleString()} indexed articles</span><span>{sourceCount} active sources</span></div>
        </div>
      </div>
    </Motion.div>
  );
    ...
    <div className="grid lg:grid-cols-[1.1fr_.9fr] border border-ink/20 dark:border-paper/20 bg-paper-card dark:bg-paper-dark-card">
      ...
      <div className="p-6 sm:p-9 min-h-72 flex flex-col justify-center">
        <AnimatePresence mode="wait">{loading ? <Motion.div key="load" exit={{opacity:0}} className="space-y-4"><Motion.div animate={{x:['-100%','250%']}} transition={{repeat:Infinity,duration:1}} className="h-1 w-1/3 bg-accent"/><p className="font-display text-2xl italic">Reading narrative signals…</p></Motion.div> : result && <Motion.div key={text+result.sentiment} initial={{opacity:0,y:15}} animate={{opacity:1,y:0}}>
          <p className="text-[10px] uppercase tracking-[.22em] text-ink-faint dark:text-[#B8B3A7]">Analysis result</p><div className="flex items-end gap-3 mt-3"><span className="font-display text-4xl font-bold" style={{color}}>{result.sentiment}</span><span className="mb-1 text-xs font-bold">{result.confidence}% confidence</span></div>
          ...
          <div className="mt-7 grid grid-cols-2 gap-5"><div><p className="text-[9px] uppercase tracking-widest text-ink-faint dark:text-[#B8B3A7]">Detected entities</p><p className="mt-2 text-sm font-semibold">{result.entities.join(' · ')}</p></div><div><p className="text-[9px] uppercase tracking-widest text-ink-faint dark:text-[#B8B3A7]">Tone cues</p><p className="mt-2 text-sm font-semibold">{result.cues.length ? result.cues.join(' · ') : 'Factual · balanced'}</p></div></div>
...
const Footer = () => <footer className="border-t-2 border-ink dark:border-paper"><div className="max-w-7xl mx-auto px-6 py-12 flex flex-col md:flex-row justify-between gap-8"><div><p className="font-display text-xl font-bold">MY News <i className="text-accent">Sentiment</i></p><p className="text-xs text-ink-faint dark:text-[#B8B3A7] mt-2">Malaysia’s media intelligence layer.</p></div><div className="flex flex-wrap gap-6 text-xs uppercase tracking-wider"><Link to="/features">Features</Link><Link to="/api-docs">API</Link><Link to="/about">About</Link><Link to="/contact">Contact</Link><Link to="/privacy">Privacy</Link></div></div></footer>;
  ...
  return <div className="min-h-screen bg-paper dark:bg-paper-dark text-ink dark:text-paper overflow-x-hidden">
    ...
    <header className="relative pt-28 sm:pt-32 pb-20 px-5 sm:px-6 overflow-hidden">
      ...
      <Motion.div initial="hidden" animate="visible" variants={stagger} className="relative max-w-7xl mx-auto">
        <Motion.div variants={fadeUp} className="flex items-center justify-between border-y border-ink/20 dark:border-paper/20 py-2 mb-10 text-[9px] sm:text-[10px] tracking-[.2em] text-ink-muted dark:text-ink-faint dark:text-[#B8B3A7]"><span>VOL. I · NO. 01</span><span>KUALA LUMPUR · MALAYSIA</span><span className="hidden sm:block">AI MEDIA INTELLIGENCE</span></Motion.div>
        <div className="grid lg:grid-cols-[.86fr_1.14fr] gap-12 lg:gap-16 items-center">
          <div>
            ...
            <Motion.p variants={fadeUp} className="mt-7 max-w-xl text-base sm:text-lg leading-8 text-ink-muted dark:text-ink-faint dark:text-[#B8B3A7] font-serif">Real-time AI intelligence across Malaysian news—sentiment, source framing, entities and emerging narratives in one editorial command centre.</Motion.p>
            <Motion.div variants={fadeUp} className="mt-8 flex flex-col sm:flex-row gap-3"><MagneticButton onClick={()=>navigate('/dashboard')} className="group whitespace-nowrap px-5 py-3.5 bg-ink dark:bg-paper text-paper dark:text-ink text-[11px] font-bold tracking-[.1em] uppercase hover:bg-accent dark:hover:bg-accent dark:hover:text-paper">Explore live dashboard <Motion.span variants={{hover:{x:5}}} className="inline-block"><ArrowRight className="inline w-4 h-4 ml-1"/></Motion.span></MagneticButton><Motion.a whileHover={{y:-2}} href="#analyzer" className="whitespace-nowrap px-5 py-3.5 border-2 border-ink dark:border-paper text-[11px] font-bold tracking-[.1em] uppercase text-center hover:border-accent hover:text-accent">Analyze a headline <Motion.span animate={{rotate:[0,8,-8,0]}} transition={{duration:2.5,repeat:Infinity,repeatDelay:1.5}} className="inline-block"><Sparkles className="inline w-4 h-4 ml-1"/></Motion.span></Motion.a></Motion.div>
            <Motion.p variants={fadeUp} className="mt-4 text-[10px] uppercase tracking-widest text-ink-faint dark:text-[#B8B3A7]"><Check className="inline w-3 h-3 text-[#16855b] mr-1"/> No account required to explore</Motion.p>
          </div>
